        ...
        <View style={styles.row}>
            ...
            <View style={styles.amountRow}>
                <Text
                    style={[
                        styles.amount,
                        highlightLabel && styles.highlightAmount,
                    ]}
                >
    ...
    useEffect(() => {
        const fetchTransactions = async () => {
            const { data, error } = await supabase
                .from("coin_logs")
                .select("timestamp, coin_value"); // Changed from daily_total to coin_value
            ...
            if (selectedTimeframe === "Daily") {
                ...
                data.forEach(({ timestamp, coin_value }) => {
                    // Changed from daily_total to coin_value
                    const date = dayjs(timestamp).format("YYYY-MM-DD");
                    grouped[date] = (grouped[date] || 0) + coin_value; // Changed from daily_total to coin_value
                });
                ...
                data.forEach(({ timestamp, coin_value }) => {
                    ...
                    grouped[key] = (grouped[key] || 0) + coin_value; // Changed from daily_total to coin_value
                });
